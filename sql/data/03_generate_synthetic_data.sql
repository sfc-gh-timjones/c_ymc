...
-- EMPLOYEES (500 rows) — FK to BRANCHES (1-20), DEPARTMENTS (1-15)
...
SELECT
    ...
    CASE WHEN UNIFORM(1, 10, RANDOM()) <= 8 THEN UNIFORM(1, 50, RANDOM()) ELSE NULL END AS MANAGER_ID,
    CASE WHEN UNIFORM(1, 100, RANDOM()) <= 92 THEN TRUE ELSE FALSE END AS IS_ACTIVE
FROM TABLE(GENERATOR(ROWCOUNT => 500));

-- MEMBERS (800 rows) — FK to BRANCHES (1-20), MEMBERSHIP_TYPES (1-8)
...
-- LEADS (300 rows) — FK to BRANCHES (1-20)
...
-- CHILDCARE_PROGRAMS (40 rows) — FK to BRANCHES (1-20)
...
-- CHILDCARE_ENROLLMENTS (600 rows) — FK to CHILDCARE_PROGRAMS (1-40)
...
SELECT
    UNIFORM(1, 40, RANDOM()) AS PROGRAM_ID,
    CASE UNIFORM(1, 15, RANDOM())
...
-- IT_TICKETS (500 rows) — FK to EMPLOYEES (1-500), BRANCHES (1-20)
...
SELECT
    'TKT-' || LPAD(SEQ4()::VARCHAR, 5, '0') AS TICKET_NUMBER,
    CASE UNIFORM(1, 20, RANDOM())
    ...
    UNIFORM(1, 500, RANDOM()) AS EMPLOYEE_ID,
    UNIFORM(1, 20, RANDOM()) AS BRANCH_ID,
    CASE UNIFORM(1, 6, RANDOM())
...
-- ONBOARDING_TASKS (1000 rows) — FK to EMPLOYEES (1-500)
